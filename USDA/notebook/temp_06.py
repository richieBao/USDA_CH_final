...
parser.add_argument('--population_seeker', default=1, type=int,help='智能体搜寻着（agent_seeker）初始化数量，取值范围 1~750')
...
class sprawl_model(mesa.Model):
    def __init__(self,
        ...
        attraction_colors=self.generate_attraction_colors(attractiveness)
        # 生成吸引力地图（即智能体agent_patch）
        self.setup_agent_patch(attractiveness,attraction_colors) 
        # 初始化智能体搜寻者（seeker）
        self.setup_agent_seeker(population_seeker,(width//2,height//2)) 
    ...
    def setup_agent_seeker(self,population_seeker,pos):
        '''初始化智能体搜寻者（seeker）'''
        print(pos)
        for i in range(population_seeker):
            turtle_seeker=agent_seeker(pos,i,self) 
...
def agent_portrayal(agent):
    seeker_house_colors={0:'blue',1:'red'}
    ...
    if type(agent) is agent_patch:
        ...
        portrayal = {
            "Shape": "circle",
            "Color": 'red',#seeker_house_colors[agent.seeker_or_house],
            "Filled": "true",
            "Layer": 2,
