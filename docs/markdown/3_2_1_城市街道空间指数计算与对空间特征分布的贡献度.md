...
每一位置点建立350m的缓冲区域，提取落于缓冲区内的POI点数据。计算每一位置点对应的POI点数量，信息熵和均衡度如图X左1、2。POI的数量越多，街道空间提供的服务越多，其中数量最多的区域为中贸广场商业圈，最少的区域位于朱雀大街北段和荐福寺路。而均衡度最大的区域在POI数量最少的区域，是因为主要的行业分类，例如购物、生活服务、美食和酒店等在该区域分布较少，减少了不同行业数量的差异。

总共有19个一级行业分类，借鉴视觉词袋构建特征映射的方法，计算每一位置点19个分类的频数即特征映射（码本映射），然后进行空间距离关联的AgglomerativeClustering层次聚类。通过最优的簇数选择，最优簇数为7， 特征贡献度如表X，以丽人（4）为界，之下行业分类较多多于之上分类的贡献度，这与居民日常生活需求相关联。通常需求越多的行业分类，其贡献度也会较高。

每一簇下，POI的总数和行业类频数会有所差异，从图可以观察到，虽然各簇的总数有明显不同，但是购物和生活服务通常占据组要成分，其次为出入口、公司企业、美食和酒店。行业分类频数的组合结构表征了每一簇的服务特点。
