...
核心的`PAPER_Urban_Street_Space_Metrics`子文件夹，放置有数据、代码等所有项目文件。注意，GitHub对上传文件大小有限制。
    ...
    │   │   │   │   ├── tour line_1.jpg
    │   │   │   │   └── tour line_2.jpg
    │   │   │   ├── tourline_img_seg_redefined_color/
    │   │   │   │   ├── tour line_0.jpg
    │   │   │   │   ├── tour line_1.jpg
    │   │   │   │   └── tour line_2.jpg
    │   │   │   ├── tourline_label_seg/
...
#### B.3.1 基于全景图不同数据格式计算城市街道视域景观指数

Yu Ye等在测量街道空间绿视率时，抓取谷歌街景（Google Street View, GSV）水平视点四个方向上的图像作为图像分割数据；Jingxian Tang等下载腾讯地图街景图像（Street View Picture, SVP）做图像分割计算对象比例。而使用全景图的不同数据类型分别用于不同类型指数的计算，可以进一步提升指数计算的精度。将等量矩形投影图用于色彩丰富度指数、关键点邻域尺度指数的计算；极坐标格式全景图用于天际线变化指数的计算；立方体型全景格式用于街道空间对象视域占比指数测量。

#### B.3.2 基于尺度不变特征转换关键点描述子定义关键点邻域尺度(i,j]区间频数指数

图像匹配是计算机视觉用于解决许多问题的一项基本研究，例如对象和场景识别、多个图像中寻找三维结构和运动追踪等。David G. Lowe提出了尺度不变特征转换，可以不受图像变形、噪音等影响，有效提取图像的特征。为进一步提取城市街道空间的特征，依据尺度不变特征转换关键点描述子定义关键点邻域尺度(i,j]区间频数指数。所有街道特征指数聚类贡献度结果证明了关键点邻域尺度是城市街道空间重要特征之一，可以有效识别不同城市街道区域及确定相应关键点邻域尺度的大小区间。

#### B.3.3 视域景观指数作用于不同邻里尺度的特征效应

景观指数通常用于城市空间研究的不同方面，根据研究内容的不同选取单一指数或多个景观指数的组合进行分析。例如在研究城市街道绿化，通过绿视率量化时，可以通过聚类的途径尽量将绿视率接近的位置归为一个区域，便于宏观统筹管理。

当考虑指数的邻里尺度效应时，更高的尺度会让聚类的结果更细化，因此根据宏观调控的要求可以确定不同的邻里尺度，满足弹性的管理需求。

根据不同邻里尺度景观指数贡献度，纵观20个层级，聚类贡献度前5个为Seg_GVI,Seg_SVF, KPSF (0–10],  KPSF (10–20],和CRI，可确定为特征重要指数组成。其中SVF和GVI是视域空间天空和植被占比，为视域空间的组成成分；KPSF (0-10]和KPSF (10-20]是关键点邻域尺度区间频数，反应了视域对象特征尺度的变化，并由较小尺度区间决定；CRI是反应视域空间颜色混合特征的色彩丰富度指数。因此得出城市街道空间的组成、对象尺度与颜色是构成视域空间的重要特征指数。而反应天空形状的Sky_NP, Sky_PARA(mn), Sky_SHAPE(mn), Sky_FRAC(mn)等指数，地面占比Seg_GVF，较大特征关键点尺度区间的KPSF (20–30], KPSF(30–40]，以及分割对象Seg_ED指数对特征指数聚类贡献不明显。

#### B.3.4 局限性

PASS全景图语义分割的均交并比（Mean Intersection-Over-Union，mIoU）约为54.3%，全景图的分辨率为1024×512，同时用于拍摄全景图的车辆占据了地面方向较大的空间，均会一定程度上影响图像语义分割和特征指数计算结果的精度。区域尺度全景图是延道路每200m为一个采样点，较大距离的采样点对区域尺度的特征指数聚类和特征的具体分布有一定程度影响。上述对分析结果精度有影响的因素无法完全避免，但是未来可以尝试增加密集的采样点，并通过算法的改进和方法的调整使精度进一步提升。


#### B.3.5 结论

以全景静态图数据为分析的基础，通过当前计算机视觉发展的相关算法，解析反应城市街道的图像内容，包括语义分割、尺度不变特征转换来计算绿视率、天空视域因子、地面视域占比、视觉（熵）均衡度，主题色、色彩丰富度指数，及天空形状的周长面积比、形状指数和分维数，并提出关键点邻域尺度(i,j]区间频数和标准差等指数。应用能够反应城市街道特征的视域景观指数，执行层次聚类研究城市街道空间的特征分布和重要特征指数组成，及指数作用于邻里尺度效应，可以为城市街道空间的景观质量提升、量化管理和城市微更新提供参照。

#### B.参考文献

```txt

[1] Ewing, R., & Clemente, O. (2013). Measuring urban design: Metrics for livable places. In Measuring Urban Design: Metrics for Livable Places. 
[2] Foster, S., Giles-corti, B., & Knuiman, M. (2011). Creating safe walkable streetscapes : Does house design and upkeep discourage incivilities in suburban neighbourhoods ? Journal of Environmental Psychology, 31(1), 79–88. 
[3] Vich, G., Marquet, O., & Miralles-guasch, C. (2019). Green streetscape and walking : Exploring active mobility patterns in dense and compact cities ☆. Journal of Transport & Health, 12(October 2018), 50–59. 
[4] Wu, Y., Nash, P., Barnes, L. E., Minett, T., Matthews, F. E., Jones, A., & Brayne, C. (2014). Assessing environmental features related to mental health : a reliability study of visual streetscape images. 1–10.
[5] Liu, Y., Wang, R., Lu, Y., Li, Z., Chen, H., Cao, M., Zhang, Y., & Song, Y. (2020). Urban Forestry & Urban Greening Natural outdoor environment , neighbourhood social cohesion and mental health : Using multilevel structural equation modelling , streetscape and remote-sensing metrics. 48(July 2019). 
[6] Seiferling, I., Naik, N., Ratti, C., & Proulx, R. (2017). Landscape and Urban Planning Green streets − Quantifying and mapping urban trees with street-level imagery and computer vision. 165(May), 93–101. 
[7] Cain, K. L., Millstein, R. A., Sallis, J. F., Conway, T. L., Gavand, K. A., Frank, L. D., Saelens, B. E., Geremia, C. M., Chapman, J., Adams, M. A., Glanz, K., & King, A. C. (2014). Social Science & Medicine Contribution of streetscape audits to explanation of physical activity in four age groups based on the Microscale Audit of Pedestrian Streetscapes ( MAPS ). Social Science & Medicine, 116, 82–92. 
[8] Sarkar, C., Webster, C., Pryor, M., Tang, D., Melbourne, S., Zhang, X., & Jianzheng, L. (2015). Landscape and Urban Planning Exploring associations between urban green , street design and walking : Results from the Greater London boroughs. Landscape and Urban Planning, 143, 112–125. 
[9] Ewing, R., Hajrasouliha, A., Neckerman, K. M., Purciel-Hill, M., & Greene, W. (2016). Streetscape Features Related to Pedestrian Activity. Journal of Planning Education and Research, 36(1), 5–15. 
[10] Hanibuchi, T. (2019). Health & Place Virtual audits of streetscapes by crowdworkers. Health & Place, 59(May), 102203. 
[11] Taylor, P., Spokane, A. R., Lombard, J. L., Martinez, F., Mason, C. A., Gorman-smith, D., Plater-zyberk, E., Brown, S. C., Perrino, T., Szapocznik, J., Spokane, A. R., Lombard, J. L., Martinez, F., Mason, C. A., Gorman-smith, D., Plater-zyberk, E., & Brown, S. C. (2011). Identifying Streetscape Features Significant to Well- Being Identifying Streetscape Features Significant to Well-Being. July 2014, 37–41. 
[12] Wijnands, J. S., Nice, K. A., Thompson, J., Zhao, H., & Stevenson, M. (2019). Streetscape augmentation using generative adversarial networks : Insights related to health and wellbeing. Sustainable Cities and Society, 49(May), 101602. 
[13] Curl, A., Ward, C., & Aspinall, P. (2015). Landscape and Urban Planning The effectiveness of ‘ shared space ’ residential street interventions on self-reported activity levels and quality of life for older people. Landscape and Urban Planning, 139, 117–125. 
[14] Said, S., & Samadi, Z. (2016). The Evolution of Historic Streetscape in Adapting Modern Demand in Achieving the Quality of Life. Procedia - Social and Behavioral Sciences, 234, 488–497. 
[15] Harvey, C., Aultman-hall, L., Hurley, S. E., & Troy, A. (2015). Landscape and Urban Planning Effects of skeletal streetscape design on perceived safety. Landscape and Urban Planning, 142, 18–28. 
[16] Naik, N., & Philipoom, J. (2014). Streetscore - Predicting the Perceived Safety of One Million Streetscapes. 0–6. 
[17] Tang, J., & Long, Y. (2019). Landscape and Urban Planning Measuring visual quality of street space and its temporal variation : Methodology and its application in the Hutong area in Beijing. 191(October 2018). 
[18] Preferences, S. (2011). Indoor and Built Environment Indoor and B uilt Seasonal Preferences and Interactions of Visual / Auditory Elements of Streetscape in Namwon City , Korea. 
[19] Klemm, W., Heusinkveld, B. G., Lenzholzer, S., & Hove, B. Van. (2015). Landscape and Urban Planning Street greenery and its physical and psychological impact on thermal comfort. Landscape and Urban Planning, 138, 87–98.
[20] Harvey, C., & Aultman-hall, L. (2015). Urban Streetscape Design and Crash Severity. 2500, 1–8. 
[21] Assistant, T. P., Apparicio, P., Landry, S., & Lewnard, J. (2017). Landscape and Urban Planning Disentangling the effects of urban form and socio-demographic context on street tree cover : A multi-level analysis from Montréal. Landscape and Urban Planning, 157, 422–433. 
[22] Thornton, C. M., Conway, T. L., Cain, K. L., Gavand, K. A., Saelens, B. E., Frank, L. D., Geremia, C. M., Glanz, K., King, A. C., & Sallis, J. F. (2016). SSM -Population Health Disparities in pedestrian streetscape environments by income and race / ethnicity. SSM - Population Health, 2, 206–216. 
[23] Lee, I., Jung, S., Lee, J., & Macdonald, E. (2017). Street crime prediction model based on the physical characteristics of a streetscape : Analysis of streets in low-rise housing areas in South Korea. 
[24] Ye, Y., Richards, D., Lu, Y., Song, X., Zhuang, Y., Zeng, W., & Zhong, T. (2018). Landscape and Urban Planning Measuring daily accessed street greenery : A human-scale approach for informing better urban planning practices. July 2017. 
[25] Li, X., Ratti, C., & Seiferling, I. (2018). Landscape and Urban Planning Quantifying the shade provision of street trees in urban landscape : A case study in Boston , USA , using Google Street View. Landscape and Urban Planning, 169(August 2017), 81–91. 
[26] Badland, H. M., Opit, S., Witten, K., Kearns, R. A., & Mavoa, S. (2010). Can Virtual Streetscape Audits Reliably Replace Physical Streetscape Audits ? 87(6), 1007–1016. 
[27] Zhu, W., Sun, Y., Kurka, J., Geremia, C., Engelberg, J. K., Cain, K., Conway, T., Sallis, J. F., Hooker, S. P., & Adams, M. A. (2017). Landscape and Urban Planning Reliability between online raters with varying familiarities of a region : Microscale Audit of Pedestrian Streetscapes ( MAPS ). Landscape and Urban Planning, 167(June), 240–248. 
[28] Vries, S. De, Dillen, S. M. E. Van, Groenewegen, P. P., & Spreeuwenberg, P. (2013). Social Science & Medicine Streetscape greenery and health : Stress , social cohesion and physical activity as mediators. Social Science & Medicine, 94, 26–33. 
[29] Yang, K., Hu, X., Bergasa, L. M., Romera, E., & Wang, K. (2020). PASS: Panoramic Annular Semantic Segmentation. IEEE Transactions on Intelligent Transportation Systems, 21(10), 4171–4185. 
[30] Zhou, C., Li, Z., & Liu, Y. (2017). A measurement study of oculus 360 degree video streaming. Proceedings of the 8th ACM Multimedia Systems Conference, MMSys 2017, 27–37. 
[31] Zhang, J., Heng, C. K., Malone-Lee, L. C., Hii, D. J. C., Janssen, P., Leung, K. S., & Tan, B. K. (2012). Evaluating environmental implications of density: A comparative case study on the relationship between density, urban block typology and sky exposure. Automation in Construction, 22, 90–101. 
[32] Ki, D., & Lee, S. (2021). Analyzing the effects of Green View Index of neighborhood streets on walking time using Google Street View and deep learning. Landscape and Urban Planning, 205(January 2020), 103920. 
[33] He, X., Miao, S., Shen, S., Li, J., Zhang, B., Zhang, Z., & Chen, X. (2015). Influence of sky view factor on outdoor thermal environment and physiological equivalent temperature. International Journal of Biometeorology, 59(3), 285–297. 
[34] Chen, L., Ng, E., An, X., Ren, C., Lee, M., Wang, U., & He, Z. (2012). Sky view factor analysis of street canyons and its implications for daytime intra-urban air temperature differentials in high-rise, high-density urban areas of Hong Kong: A GIS-based simulation approach. International Journal of Climatology, 32(1), 121–136. 
[35] Middel, A., Lukasczyk, J., Maciejewski, R., Demuzere, M., & Roth, M. (2018). Sky View Factor footprints for urban climate modeling. Urban Climate, 25(February), 120–134. 
...
在项目下建立`setup.py`安装文件，包括内容如下，可以配置项目名、版本、作者和文件位置列表（package）。在anaconda下建立环境，对应终端（terminal）输入`pip install -e . --user`，完成该项目的安装。安装之后，则可以在该环境下调用该项目的所有函数和类。

`setup.py`
```python
# -*- coding: utf-8 -*-
"""
Created on Wed Jan 19 10:13:29 2022
...
setup(name='PAPER_Urban_Street_Space_Metrics',
      version='0.1',
      author='Richie Bao-caDesign设计(cadesign.cn)',
      packages=['PAPER_Urban_Street_Space_Metrics'])
```

同时在项目下建立`tests`文件夹，复制数据文件，通过编写代码文件`test.py`，测试是否安装成功。

`test.py`
```python
# -*- coding: utf-8 -*-
"""
Created on Tue Feb  1 22:41:25 2022

@author: Richie Bao-caDesign设计(cadesign.cn)
"""

from PAPER_Urban_Street_Space_Metrics import database as db
from PAPER_Urban_Street_Space_Metrics.analysis_n_results import visual_field_objects_position_changes as vfopc
...
if __name__=="__main__":
    ...
    import sys,os,glob    
    parent_path=os.path.dirname(os.getcwd())
    cfg=db.cfg_load_yaml('./config.yml')  
    ...
    vfopc.spherical_img_pts_show('./img_object_change.jpg',FOV=False)
```

如果希望卸载该项目，则在终端执行`pip uninstall .`实现卸载。

该项目可以从GitHub仓库[PAPER_Urban_Street_Space_Metrics](https://github.com/richieBao/PAPER_Urban_Street_Space_Metrics)，`https://github.com/richieBao/PAPER_Urban_Street_Space_Metrics`下获取。
